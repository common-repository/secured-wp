...
use WPSEC_Vendor\Endroid\QrCode\ImageData\LabelImageData;
use WPSEC_Vendor\Endroid\QrCode\ImageData\LogoImageData;
use WPSEC_Vendor\Endroid\QrCode\Label\Alignment\LabelAlignmentLeft;
use WPSEC_Vendor\Endroid\QrCode\Label\Alignment\LabelAlignmentRight;
...
use WPSEC_Vendor\Endroid\QrCode\Writer\Result\PngResult;
use WPSEC_Vendor\Endroid\QrCode\Writer\Result\ResultInterface;
use WPSEC_Vendor\Zxing\QrReader;
/** @internal */
final class PngWriter implements WriterInterface, ValidatingWriterInterface
{
    public function write(QrCodeInterface $qrCode, LogoInterface $logo = null, LabelInterface $label = null, array $options = []) : ResultInterface
    {
        ...
        $backgroundColor = \imagecolorallocatealpha($baseImage, $qrCode->getBackgroundColor()->getRed(), $qrCode->getBackgroundColor()->getGreen(), $qrCode->getBackgroundColor()->getBlue(), $qrCode->getBackgroundColor()->getAlpha());
        \imagefill($baseImage, 0, 0, $backgroundColor);
        for ($rowIndex = 0; $rowIndex < $matrix->getBlockCount(); ++$rowIndex) {
            for ($columnIndex = 0; $columnIndex < $matrix->getBlockCount(); ++$columnIndex) {
                if (1 === $matrix->getBlockValue($rowIndex, $columnIndex)) {
                    \imagefilledrectangle($baseImage, $columnIndex * $baseBlockSize, $rowIndex * $baseBlockSize, ($columnIndex + 1) * $baseBlockSize, ($rowIndex + 1) * $baseBlockSize, $foregroundColor);
                }
            }
        }
