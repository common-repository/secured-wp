 ...
 */
if ( ! class_exists( '\WPSEC\Settings\Settings_Builder' ) ) {
	/**
	...
	class Settings_Builder {
		...
		private static function upload() {
			...
			$upload_button = ! empty( self::$settings['custom_text'] ) ? self::$settings['custom_text'] : esc_html__( 'Upload', 'secured-wp' );
			...
			<div class="image-preview-wrapper">
				<input <?php echo self::$item_id_attr; ?> <?php echo self::$name_attr; ?> class="secwp-img-path" type="text"
					value="<?php echo esc_attr( self::$current_value ); ?>" <?php echo self::$placeholder_attr; ?>>
				<input id="<?php echo 'upload_' . self::$item_id . '_button'; ?>" type="button" class="secwp-upload-img button"
		...
		private static function upload_font() {
			...
			<div class="image-preview-wrapper">
				<input <?php echo self::$item_id_attr; ?> <?php echo self::$name_attr; ?> class="secwp-font-path" type="text"
					value="<?php echo esc_attr( self::$current_value ); ?>" <?php echo self::$placeholder_attr; ?>>
				<input id="<?php echo 'upload_' . self::$item_id . '_button'; ?>" type="button" class="secwp-upload-font button"
					value="<?php echo $upload_button; ?>">
		...
		private static function checkbox() {
			...
			$toggle_data  = ! empty( self::$settings['toggle'] ) ? 'data-secwp-toggle="' . self::$settings['toggle'] . '"' : '';
			$toggle_class = ! empty( self::$settings['toggle'] ) ? 'secwp-toggle-option' : '';

			?>
				<input <?php echo self::$item_id_attr; ?> <?php echo self::$name_attr; ?>
					class="secwp-js-switch <?php echo $toggle_class; ?>" <?php echo $toggle_data; ?> type="checkbox" value="true"
							<?php echo $checked; ?>>
							<?php
		}
		...
		private static function color() {
			...
			$custom_class = ! empty( self::$settings['color_class'] ) ? self::$settings['color_class'] : '';

			?>

			<div class="secwp-custom-color-picker">
		...
		private static function posts() {
			?>
			<div class="secwp-custom-posts-selector">
		...
		private static function post() {
			?>
			<div class="secwp-custom-post-selector">
		...
		private static function fonts() {
			?>
			<input <?php echo self::$name_attr; ?> <?php echo self::$item_id_attr; ?> class="secwp-select-font" type="text" value="<?php echo esc_attr( self::$current_value ); ?>">
			<?php
		}
		...
		private static function notice_message() {
			...
			self::$custom_class .= ' secwp-message-hint';

			if ( 'error' === self::$option_type ) {
				self::$custom_class .= ' secwp-message-error';
			} elseif ( 'success' === self::$option_type ) {
				self::$custom_class .= ' secwp-message-success';
			}
		...
		private static function number() {
			?>
			<input style="width:60px" min="<?php echo self::$min; ?>" max="<?php echo self::$max; ?>" <?php echo self::$item_id_attr; ?> <?php echo self::$name_attr; ?> type="number" value="<?php echo esc_attr( self::$current_value ); ?>" <?php echo self::$placeholder_attr; ?>>
			<?php
		}
		...
		private static function section_head() {
			...
			<h3 <?php echo self::$item_id_attr; ?> class="secwp-section-title <?php echo self::$custom_class; ?>">
			<?php

			echo self::$settings['title'];

			if ( ! empty( self::$settings['id'] ) ) {
				\do_action( 'secwp_admin_after_head_title', self::$settings['id'] );
			}
			?>
			</h3>
		...
		private static function option_head() {
			...
			if ( ! empty( self::$settings['pre_text'] ) ) {
				?>
				<div class="secwp-option-pre-label"><?php echo self::$settings['pre_text']; ?></div>
				<div class="clear"></div>
				<?php
			}
			...
			if ( ! empty( self::$settings['name'] ) ) {
				?>
				<span class="secwp-label"><?php echo self::$settings['name']; ?></span>
				<?php
			}
		}
		...
		private static function visual() {
			?>
			<ul id="secwp_<?php echo self::$item_id; ?>" class="secwp-options">
			...
			jQuery(document).ready(function() {
				...
				jQuery(document).on('click', '#secwp_<?php echo esc_js( self::$item_id ); ?> a', function() {
		...
		private static function gallery() {
			...
			class="secwp-upload-image secwp-primary-buttonton button button-primary button-large"
			value="<?php esc_html_e( 'Add Image', 'secured-wp' ); ?>">

			<ul id="<?php echo esc_attr( self::$item_id ); ?>-gallery-items" class="secwp-gallery-items">
				<?php
				...
				<li id="listItem_<?php echo esc_attr( $counter ); ?>" class="ui-state-default">
					<div class="gallery-img img-preview">
						...
						<input id="secwp_post_gallery[<?php echo esc_attr( $counter ); ?>][id]"
							name="secwp_post_gallery[<?php echo esc_attr( $counter ); ?>][id]"
			...
			jQuery(document).ready(function() {
				...
				// Uploading files
				var secwp_slider_uploader;

				jQuery(document).on('click', '#<?php echo esc_attr( self::$item_id ); ?>-upload', function(
					event) {
					event.preventDefault();
					secwp_slider_uploader = wp.media.frames.secwp_slider_uploader = wp.media({
						title: '<?php esc_html_e( 'Add Image', 'secured-wp' ); ?>',
						library: {
							type: 'image'
						},
						button: {
							text: '<?php esc_html_e( 'Select', 'secured-wp' ); ?>'
						},
						multiple: true,
					});

					secwp_slider_uploader.on('select', function() {
						var selection = secwp_slider_uploader.state().get('selection');
							...
							jQuery(
								...
								.append('\
										<li id="listItem_' + nextImgCell + '" class="ui-state-default">\
											<div class="gallery-img img-preview">\
												<img src="' + attachment.url + '" alt=""><input id="secwp_post_gallery[' +
									nextImgCell + '][id]" name="secwp_post_gallery[' +
		...
		private static function background() {
			...
			<input id="<?php echo esc_attr( self::$item_id ); ?>-img" class="secwp-img-pasecwp-igaro-background-path"
			...
			value="<?php esc_html_e( 'Upload', 'secured-wp' ); ?>">

			<div class="secwp-background-options">
				...
				<select name="<?php echo esc_attr( self::$option_name ); ?>[repeat]"
					...
					?>
					><?php esc_html_e( 'no-repeat', 'secured-wp' ); ?></option>
					<option value="repeat" 
					<?php
					if ( ! empty( $current_value['repeat'] ) ) {
						selected( $current_value['repeat'], 'repeat' );}
					?>
					><?php esc_html_e( 'Tile', 'secured-wp' ); ?></option>
					<option value="repeat-x" 
					<?php
					if ( ! empty( $current_value['repeat'] ) ) {
						selected( $current_value['repeat'], 'repeat-x' );}
					?>
					><?php esc_html_e( 'Tile Horizontally', 'secured-wp' ); ?></option>
					<option value="repeat-y" 
					<?php
					if ( ! empty( $current_value['repeat'] ) ) {
						selected( $current_value['repeat'], 'repeat-y' );}
					?>
					><?php esc_html_e( 'Tile Vertically', 'secured-wp' ); ?></option>
				</select>
				...
				<select name="<?php echo esc_attr( self::$option_name ); ?>[attachment]"
					...
					?>
					><?php esc_html_e( 'Fixed', 'secured-wp' ); ?></option>
					<option value="scroll" 
					<?php
					if ( ! empty( $current_value['attachment'] ) ) {
						selected( $current_value['attachment'], 'scroll' );}
					?>
					><?php esc_html_e( 'Scroll', 'secured-wp' ); ?></option>
					<option value="cover" 
					<?php
					if ( ! empty( $current_value['attachment'] ) ) {
						selected( $current_value['attachment'], 'cover' );}
					?>
					><?php esc_html_e( 'Cover', 'secured-wp' ); ?></option>
				</select>
				...
				<select name="<?php echo esc_attr( self::$option_name ); ?>[hor]"
					...
					?>
					><?php esc_html_e( 'Left', 'secured-wp' ); ?></option>
					<option value="right" 
					<?php
					if ( ! empty( $current_value['hor'] ) ) {
						selected( $current_value['hor'], 'right' );}
					?>
					><?php esc_html_e( 'Right', 'secured-wp' ); ?></option>
					<option value="center" 
					<?php
					if ( ! empty( $current_value['hor'] ) ) {
						selected( $current_value['hor'], 'center' );}
					?>
					><?php esc_html_e( 'Center', 'secured-wp' ); ?></option>
				</select>
				...
				<select name="<?php echo esc_attr( self::$option_name ); ?>[ver]"
					...
					?>
					><?php esc_html_e( 'Top', 'secured-wp' ); ?></option>
					<option value="bottom" 
					<?php
					if ( ! empty( $current_value['ver'] ) ) {
						selected( $current_value['ver'], 'bottom' );}
					?>
					><?php esc_html_e( 'Bottom', 'secured-wp' ); ?></option>
					<option value="center" 
					<?php
					if ( ! empty( $current_value['ver'] ) ) {
			...
			<div id="<?php echo esc_attr( self::$item_id ); ?>-preview" class="img-preview" 
				...
				<img src="
					...
					?>
					" alt="">
				<a class="del-img" title="<?php esc_html_e( 'Remove', 'secured-wp' ); ?>"></a>
			</div>
		...
		private static function typography() {
			...
			<div class="secwp-custom-select typography-custom-select">
				<select name="<?php echo esc_attr( self::$option_name ); ?>[size]"
					...
					<option <?php selected( $current_value['size'], '' ); ?> <?php disabled( 1, 1 ); ?>>
						<?php esc_html_e( 'Font Size in Pixels', 'secured-wp' ); ?></option>
					<option value=""><?php esc_html_e( 'Default', 'secured-wp' ); ?></option>
					<?php for ( $i = 8; $i < 61; $i++ ) { ?>
			...
			<div class="secwp-custom-select typography-custom-select">
				<select name="<?php echo esc_attr( self::$option_name ); ?>[line_height]"
					...
					<option <?php selected( $current_value['line_height'], '' ); ?> <?php disabled( 1, 1 ); ?>>
						<?php esc_html_e( 'Line Height', 'secured-wp' ); ?></option>
					<option value=""><?php esc_html_e( 'Default', 'secured-wp' ); ?></option>
		...
		private static function prepare_data( $settings, $option_name, $data ) {
			...
			self::$min = ! empty( $min ) ? $min : -1000;
			self::$max = ! empty( $max ) ? $max : 1000000;
